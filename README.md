...
### Sample queries it handles well

- *"Which sleeping pad has the highest R-value?"* → ThermoMat Insulated Air Mat (R-5.5)
- *"What's the cheapest climbing harness?"* → SummitGrip Climbing Harness ($85)
- *"Compare the kayaks available."* → SeaScout Inflatable Kayak vs. WaveRider SUP Board with specs and price contrast
- *"Which tent is the most waterproof?"* → Alpine Explorer Tent (3000mm rainfly), with comparison to TrailMaster X4 (2000mm)
- *"Show me items under $25."* → Cross-category filtered list with prices
- *"Do you sell snowshoes?"* → SnowTrek Snowshoes; bot also correctly distinguishes from non-snowshoe winter gear
...
Screenshots from the deployed Azure infrastructure:

| | |
|---|---|
| ![Foundry model deployments](./screenshots/100products_foundry.png) | ![AI Search indexer](./screenshots/100products_search_indexer.png) |
| Azure AI Foundry — `gpt-4o` and `text-embedding-ada-002` deployed | AI Search indexer — 100/100 documents succeeded, 0 errors |
| ![AI Search index](./screenshots/100products_search_index.png) | ![Resource group](./screenshots/resource_group.png) |
| Vector index `rag-100products` — chunked and embedded | All resources in `azureAI` resource group |
